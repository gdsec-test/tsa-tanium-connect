...
```<Module>``` refers to the Tanium Module where that configuration was created.
  ...
  * A rule of thumb in deciding how to word this variable is whether or not a different configuration type might be part of a different Connect Job where an administrator would want to know the difference between them.
  * Patch Lists are one example:
    ...
    * Given the Connect Jobs actually call SQs and Asset Views, they never refer to the Patch Lists directly.
